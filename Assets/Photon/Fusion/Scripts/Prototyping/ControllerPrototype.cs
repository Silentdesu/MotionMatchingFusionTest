...
public class ControllerPrototype : Fusion.NetworkBehaviour {
  ...
  protected MxMTrajectoryGenerator _trajectoryGenerator = default;
  ...
  private void CacheComponents() {
    ...
    if (!_trajectoryGenerator) _trajectoryGenerator = GetComponent<MxMTrajectoryGenerator>();
  }
  ...
  public override void FixedUpdateNetwork() {
    ...
    if (GetInput(out NetworkInputPrototype input)) {
      ...
      if (input.IsDown(NetworkInputPrototype.BUTTON_FORWARD)) {
        direction += Vector3.Lerp(direction, TransformLocal ? transform.forward : Vector3.forward, Runner.DeltaTime);
      }
    ...
    if (_ncc) {
      _trajectoryGenerator?.SetInput(new Vector2(direction.x, direction.z));
      _ncc.Move(direction);
